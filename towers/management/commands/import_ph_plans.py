...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):        
        ...
        items = TowerBedroomTypes.objects.all().order_by('id')
        sort_order = 0
        for item in items:
        ...
        for tower in data:            
            ...
            print(tower['title'])
            tower_instance = Tower.objects.get(title=tower['title'])                
            sort_order = len(tower_instance.plans.all())
            ...
            for item in plans['values'][1:]:
                ...
                room_type_so = 0
                if 'SPH' in item[2]:
                    room_type_title = 'Sub Penthouse'
                    room_type_so = 10
                elif 'PH' in item[2]:
                    room_type_title = 'Penthouse'    
                    room_type_so = 11
                else:
                    continue    

                try:                        
                    bt = TowerBedroomTypes.objects.get(title=room_type_title)
                except TowerBedroomTypes.DoesNotExist:
                    bt = TowerBedroomTypes(
                        title=room_type_title,
                        title_zh_hans=room_type_title,
                        title_zh_hant=room_type_title,
                        sort_order=room_type_so,
                        visible=False,
                    )
                    bt.save()
